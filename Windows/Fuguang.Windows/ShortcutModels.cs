...
internal sealed class ShortcutBinding
{
    ...
    {
        Key = string.IsNullOrWhiteSpace(Key) ? fallbackKey : Key;
        if (Kind == ShortcutActionKind.VoiceInput)
        {
            Kind = ShortcutActionKind.Screenshot;
        }
        ...
        get
        {
            ...
            return Kind switch
            {
                ...
                ShortcutActionKind.OpenApplication or ShortcutActionKind.OpenFolder or ShortcutActionKind.ImageQuickLook
                    => Path.GetFileNameWithoutExtension(target.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)) ?? Kind.Title(),
                ShortcutActionKind.OpenWebsite => string.IsNullOrWhiteSpace(target) ? Kind.Title() : target,
                _ => Kind.Title()
            };
        }
    }
